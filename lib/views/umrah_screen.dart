...
import '../controllers/umrah_controller.dart';
import 'constants/appcolors.dart';
import 'constants/appimages.dart';
import 'widgets/animated_button.dart';

class UmrahScreen extends StatelessWidget {
  UmrahScreen({super.key});

  final UmrahController controller =
      Get.put(UmrahController(), permanent: true);

  @override
  Widget build(BuildContext context) {
    Future.microtask(() => controller.loadUmrahData());
          ...
          children: [
            ...
            Column(
              ...
              children: [
                ...
                Text(
                  easy.tr('complete_guide_to_umrah'),
                  style: GoogleFonts.poppins(
                      ...
                      child: FadeInAnimation(
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildFormattedContent(content, context),
  ...
  Widget _buildFormattedContent(String content, BuildContext context) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (line.contains('|')) {
        widgets.add(_buildTableRow(line, context));
        continue;
      }

      // Check if this is a bullet point
      if (line.startsWith('- ')) {
        widgets.add(_buildBulletPoint(line.substring(2), context));
        continue;
      }

      // Check if this is a numbered item (A., B., C., etc.)
      if (RegExp(r'^[A-Z]\.\s').hasMatch(line)) {
        widgets.add(_buildNumberedItem(line, context));
        continue;
      }
  ...
  Widget _buildTableRow(String line, BuildContext context) {
    final cells = line
        .split('|')
        .map((cell) => cell.trim())
  ...
  Widget _buildNumberedItem(String line, BuildContext context) {
    return Padding(
